...
class PromotionReadOnlyProbe:
    def __init__(self, path: str):
        ...
        try:
            with open(self.path, "r", encoding="utf-8-sig") as handle:
                existing_probe = json.load(handle)
            if isinstance(existing_probe, Mapping):
                restored_templates = self._unprotect_catalog_templates(
                    existing_probe.get("catalog_templates_protected")
                )
        except Exception:
            restored_templates = []
        ...
        # Requests generated by the probe itself must never become reusable
        # browser-observed templates. A rejected replay otherwise overwrites a
        # known-good request shape and makes later refreshes fail identically.
        self._generated_catalog_request_variants = set()
        ...
        for item in restored_templates:
            if not isinstance(item, Mapping):
                continue
            scene = str(item.get("promotion_scene") or "").strip().lower()
            system = str(item.get("plan_system") or "").strip().lower()
            body = item.get("body")
            if (
        ...
        self._write(
            {
                ...
                "apis": [],
                "requests": [],
                "catalog_templates_protected": self._catalog_templates_protected,
            }
        )
    ...
    @staticmethod
    def _protect_catalog_templates(records: List[Dict[str, Any]]) -> str:
        if os.name != "nt" or not records:
            return ""
        try:
            import win32crypt
            ...
            protected = win32crypt.CryptProtectData(
                raw,
                "qcsckp-readonly-catalog-templates",
                None,
                None,
                None,
                0,
    ...
    @staticmethod
    def _unprotect_catalog_templates(value: Any) -> List[Dict[str, Any]]:
        text = str(value or "")
        if os.name != "nt" or not text.startswith("dpapi:"):
            return []
        try:
            import win32crypt

            raw = base64.b64decode(text[6:])
            clear = win32crypt.CryptUnprotectData(
    ...
    def _set_catalog_request_identity(
        ...
        *,
        ...
        if "PageSize" in result:
            result["PageSize"] = max(100, int(result.get("PageSize") or 0))
        page_params = PromotionReadOnlyProbe._request_page_params(result)
        if isinstance(page_params, dict):
            ...
            if "PageSize" in page_params:
                page_params["PageSize"] = max(
                    100, int(page_params.get("PageSize") or 0)
                )
    ...
    @staticmethod
    def _canonical_catalog_request_body(
        *,
        aavid: str,
        promotion_scene: str,
        plan_system: str,
        ...
        """Build the smallest observed read-only catalog contract.

        Qianchuan currently emits both a nested request and this canonical
        adapter request.  A live-shaped nested request cannot safely be
        converted into a product request by replacing only the dataset.
        """
        scene = str(promotion_scene or "").strip().lower()
        system = str(plan_system or "").strip().lower()
        dataset = CATALOG_CLASS_DATASET.get((scene, system), "")
        if not dataset:
            raise ValueError("不支持的计划目录分类")
        result: Dict[str, Any] = {
            "aavid": str(aavid or "").strip(),
            "Page": max(1, int(page or 1)),
            "PageSize": max(1, int(page_size or 100)),
            "MarGoal": 1 if scene == "product" else 2,
    ...
    async def fetch_catalog_class_from_backend(
        ...
        *,
        ...
        # 实际页面的直播/商品嵌套请求形状不完全相同。如果
        # 同类页面模板仍被平台拒绝，改用四类适配器已取证的
        # 最小只读契约重试，不带页面筛选或写操作字段。
        canonical_body = self._canonical_catalog_request_body(
            aavid=aid,
            promotion_scene=scene,
            plan_system=system,
        )
        ...
        *,
        ...
        if ad_ids is not None:
            ...
            rows = [
                row
                for row in rows
                if str(row.get("ad_id") or "").strip() in selected_ids
            ]
        ...
        async def fetch_detail(row: Dict[str, Any]) -> tuple[str, Any]:
            ad_id = str(row.get("ad_id") or "").strip()
            try:
                ...
                return ad_id, response
            except Exception as exc:
                return ad_id, exc

        fetched_details = dict(
            await asyncio.gather(*(fetch_detail(row) for row in rows))
        )
        for row in rows:
            ad_id = str(row.get("ad_id") or "").strip()
            try:
                response = fetched_details.get(ad_id)
        ...
        try:
            ...
            if path in PRODUCT_AD_LIST_API_PATHS:
                try:
                    ...
                    request_body = {}
                request_variant = self._catalog_variant_key(path, request_body)
                response_ok = int(response.status) == 200 and (
                    not isinstance(payload, Mapping)
                    or payload.get("status_code") in (None, 0)
                )
                if (
                    path == CATALOG_REQUIRED_PATH
                    and isinstance(request_body, dict)
                    and response_ok
                    and request_variant
                    not in self._generated_catalog_request_variants
                ):
                    template_aavid = (
                    ...
                    if template_aavid.isdigit():
                        ...
                        if (
                            ...
                            and template_system not in {"global", "chengfang"}
                            and template_scene
                            == str(
                                self._catalog_context.get("promotion_scene")
                                or ""
                            )
                        ):
                            template_system = str(
                                self._catalog_context.get("plan_system") or ""
                            )
                        if (
                            template_scene in {"live", "product"}
                            ...
                            if self._request_page_number(request_body) == 1:
                                ...
                                self._catalog_templates_protected = (
                                    self._protect_catalog_templates(
                                        self._serializable_catalog_templates()
                                    )
                                )
                        # 仅作为旧页面兼容模板，不再被后续的其他
    ...
    async def _execute_full_catalog_replay(
        ...
        *,
        ...
        async def fetch_and_record(body_to_send: Dict[str, Any]) -> Dict[str, Any]:
            generated_variant = self._catalog_variant_key(
                marker[0],
                body_to_send,
            )
            self._generated_catalog_request_variants.add(generated_variant)
