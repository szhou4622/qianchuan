...
def start_feishu_webhook_push_background_thread() -> threading.Thread | None:
    ...
    if _THREAD and _THREAD.is_alive():
        return _THREAD
    _STOP.clear()
    _THREAD = threading.Thread(target=_hourly_push_loop, name="feishu-webhook-hourly", daemon=True)
    _THREAD.start()
    print("[飞书整点推送] 后台线程已启动（每整点推送，配置：data/control_panel.json）")
    return _THREAD


def stop_feishu_webhook_push_background_thread(timeout: float = 3.0) -> None:
    global _THREAD
    _STOP.set()
    thread = _THREAD
