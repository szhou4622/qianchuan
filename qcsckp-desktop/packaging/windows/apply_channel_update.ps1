...
$manifest = Get-Content -LiteralPath (Join-Path $payloadRoot 'PACKAGE-MANIFEST.json') -Raw | ConvertFrom-Json
...
try {
    ...
    if (!$SkipRestart) {
        ...
        $home = if ($env:QCSCKP_HOME) { [IO.Path]::GetFullPath($env:QCSCKP_HOME) } else { Join-Path $env:LOCALAPPDATA 'QCSCKP' }
        $stateFile = Join-Path $home ("channels\{0}\startup-state\{1}.json" -f [string]$manifest.channel,$newProcess.Id)
        ...
        while ((Get-Date) -lt $readyDeadline) {
            ...
            if (Test-Path -LiteralPath $stateFile) {
                ...
                if ($state -and [string]$state.phase -in @('failed','window_timeout')) { throw ("New version startup failed: {0}" -f [string]$state.phase) }
                if ($state -and [string]$state.phase -eq 'ready') {
                    ...
                    $identityMatches = (
                        ...
                        [int]$state.build_revision -eq [int]$manifest.build_revision
                    )
